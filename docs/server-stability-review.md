...
- Три пары рецептов ассемблера с одинаковым выходом (`exposurechamber` и `exposure_chamber`,
  `mancore` и `man_core`, `strandcaster` и `strand_caster`) — какой применится, решает порядок в
  `HashMap`. Нужно решить, какой из пары лишний.

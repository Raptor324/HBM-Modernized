...
## BL. Проверка на боевом сервере после BK

```
Done (3.300s)! For help, type "help"
```

Ноль строк `hbm_m` с ERROR/Exception, ноль сбоев миксинов. `:1.20.1-forge:compileJava` после круга
переключения версий — чисто (12 warning'ов), `runGameTestServer` — `All 287 required tests passed`
на каждом из четырёх шагов раздела BK.

## BM. Фантомные части после сборки на Sable — ИСПРАВЛЕНО

Репорт из игры: после сборки машины на корабле Sable и последующего слома в мире остаются блоки
«Базовый корпус». Причин оказалось две, плюс ни одного механизма уборки для уже испорченных миров.

1. **Корневая (BI1).** `MultiblockExpander` проверял `BlockEntity instanceof IMultiblockController`,
   тогда как интерфейс реализуют **блоки**: расширение выделения не работало вообще, на корабль
   уезжал только контроллер, а весь футпринт частей оставался в мире осиротевшим. Исправлено
   в разделе BI.
2. **Регрессия BK5.** Добавленная проверка владельца в `destroyStructure` пропускала часть, у
   которой `getControllerPos()` не совпадает с текущим контроллером. После переноса Sable часть до
   перепривязки хранит **старую** позицию, поэтому при сломе машины пропускались как раз её
   собственные части. Теперь часть защищена, только если по её адресу стоит **живой** контроллер;
   протухший указатель не защищает.
3. **Уборка старых хвостов.** Часть, исчерпавшая ~2 минуты ретраев перепривязки, раньше оставалась
   в мире навсегда (невидимая, ломается с одного удара, ничего не дропает). Теперь она удаляет
   себя — при условии, что окно переноса контрапшена закрыто. Уже накопившиеся фантомы разберутся
   сами при загрузке чанков.

### BM2. Слом мультиблока внутри саблевела — ИСПРАВЛЕНО

Второй репорт из игры: у физического мультиблока (пусковая площадка на корабле Sable) ломается
только один блок, остальное остаётся фантомами. Два независимых пути:

- **Сломали часть.** `UniversalMachinePartBlock.onRemove` резолвит контроллер по сохранённому
  `ControllerPos`, а после переезда в саблевел там ещё лежит **до-переносная** позиция, по которой
  в новом мире воздух. Условие «контроллер найден» не выполнялось, каскад сноса не запускался —
  отламывался ровно один блок. Теперь при протухшем указателе часть сначала перерезолвит контроллер
  (`relinkOrphanedPartDeterministic`, с откатом на радиусный поиск) и только потом решает.
- **Сломали контроллер.** `destroyStructure` сносит части строго по `getRotatedPos`. Если после
  переноса часть оказалась не на ожидаемой позиции, она оставалась. Добавлена вторая волна
  `sweepPartsPointingAt`: по габаритам структуры сносятся части, чей `getControllerPos()` указывает
  именно на этот контроллер.

### BM3. Окно переноса контрапшена не закрывалось — КОРНЕВАЯ ПРИЧИНА ВСЕХ ТРЁХ РЕПОРТОВ

Третий репорт («ломается блок, который я сломал, + контроллер, а остальные остаются») разобран
по логам локальной сборки (`logs/debug.log`, сессия 09.09.2026 18:54–18:58). Диагноз однозначный:

```
18:55:47.351 [Server thread] [HBM] contraption move window opened (thread Server thread)
18:55:47.359 ... 18:55:47.367  onRemove подавлен при переносе   (сборка корабля Sable — штатно)
18:56:03.912  onRemove подавлен при переносе, universal_machine_part @ 20481030,128,20481034
18:56:04.809  onRemove подавлен ...                              (игрок ломает части по одной)
18:56:05.103 / :05.400 / :05.950 / :06.356 / :07.052   то же самое
18:56:10.005 [HBM] placeStructure suppressed (contraption move window), controller -51,77,111
18:57:01.483 [HBM] contraption move window opened   (разборка корабля)
18:57:21…25   снова подавлены сломы игрока
```

За всю сессию: **7 сообщений «window opened», 0 «window closed»** (включая архивные
`debug-*.log.gz`). То есть `ContraptionAssemblyGuard.pop()` не выполняется никогда, окно живёт
до страховочного таймаута в 30 с, и **всё, что игрок ломает или ставит в эти 30 с после
сборки/разборки корабля, обрабатывается как «перенос блоков движком»**:

- `LevelChunkSilentRemovalMixin` глушит `onRemove` наших блоков → каскад `destroyStructure`
  не запускается, ломается ровно один блок (это и есть репорт №2 и №3);
- `MultiblockStructureHelper.placeStructure` выходит рано → поставленная в этом окне машина
  остаётся одиноким контроллером без корпуса (видно в 18:56:10, и следом идут
  `Rejecting UseItemOnPacket … too far away from hit block -51,77,111` — игрок тыкает в машину,
  у которой нет структуры).

Почему `pop()` не срабатывает: окно открывается инъекциями в `SubLevelAssemblyHelper.assembleBlocks`
и `moveBlocks`, а закрывается инъекциями в `@At("RETURN")` тех же методов. В этой сборке в те же
методы миксинят ещё как минимум четыре мода (`aeronautics`, `synaxis`, `createthrusters`,
`electroenergetics`). Достаточно одному из них отменить метод на HEAD — и `RETURN` в исходном теле
недостижим, наш `pop()` не выполняется. Симметричность push/pop в такой среде принципиально
негарантируема, поэтому чинить надо не парность, а время жизни окна.

**Исправление.** `ContraptionAssemblyGuard.endServerTick()` вызывается из `TickEvent.SERVER_POST`
(`MainRegistry`) и принудительно обнуляет глубину. Любой перенос блоков движком сборки выполняется
синхронно внутри одного тика, поэтому окно, пережившее свой тик, — по определению утечка. Ущерб
от несработавшего `pop()` ограничен тем же тиком вместо 30 секунд. Страховочный таймаут оставлен.

Попутно в `UniversalMachinePartBlock.playerWillDestroy` (обе платформенные ветки) добавлена
проверка `shouldDestroyOnPartRemoved()`: этот путь сносил контроллер безусловно, игнорируя флаг,
который честно соблюдает `onRemove`. Единственный, кто его переопределяет, — разрушенный ZIRNOX
(`MachineZirnoxDestroyedBlock`): слом одной панели руин уносил всю конструкцию.

### BM4. Ревизия остальных мультиблоков после BM3

Проверка «а фикс общий или нет»: сам гвард (`ContraptionAssemblyGuard`) — общий, его щупают все пути
(`destroyStructure`, `placeStructure`, `attemptAutoRepair`, `onRemove` частей, `MachineDrops`,
`BaseMachineBlockEntity.dropInventoryContents`). Персональной оказалась вторая половина цепочки —
`onRemove` конкретных контроллеров и мультиблоки со своей логикой частей.

**Исправлено:**

- **`MachineWatzPowerplantBlock`** — `onPlace` ставил структуру 5×3×5 (74 части), `onRemove` её
  никогда не сносил. Единственный контроллер с настоящей схемой без каскада: слом контроллера
  оставлял в мире куб невидимых фантомов. Дроп инвентаря переведён с forge-only capability на
  общий `dropInventoryContents()`, добавлен `destroyStructure`.
- **`MachineDerrickBlock`, `MachinePumpjackBlock`** — структуру сносили, но 8 слотов
  (`OilDrillBaseBlockEntity`: батарея + апгрейды) исчезали. Добавлен дроп перед сносом.
- **`MachinePressBlock`, `MachineEPressBlock`** — `drops()` вызывался и на клиенте (`drops()`
  стороны не проверяет). Тело `onRemove` ограничено сервером.
- **`PylonDummyBlock`** — слом любой фиктивной части рушил ядро через `destroyBlock(core, false)`:
  пилон исчезал без дропа даже при обычном сломе игроком. В апстриме
  `BlockDummyable.onBlockHarvested` дропает станок при сломе любого дамми. Добавлен
  `playerWillDestroy` с `drop = !instabuild`; путь без игрока (взрыв) остался без дропа, как
  в апстриме. Плюс проверка окна переноса — иначе первый снятый движком дамми уничтожал пилон
  прямо внутри контрапшена.
- **`RedPylonCoreBlock`** — `fillDummies` в `onPlace` и `clearDummies` в `onRemove` теперь не
  работают внутри окна переноса (иначе ядро при обратной установке затирает клетки, которые движок
  ещё не разложил, а при сборке — выкидывает дамми из контрапшена).
- **`RBMKColumnBlock`** — снос филлеров в `onRemove` шёл без проверки окна, а создаются филлеры
  только в `setPlacedBy`, который движки сборки не вызывают. Колонна, перевезённая кораблём,
  приезжала без филлеров и навсегда теряла коллизию верхних сегментов.
- **`RBMKColumnFillerBlock`** — javadoc обещает «слом пробрасывается на колонну», а
  `playerWillDestroy` был пустым `super`. Филлер без лут-таблицы: игрок выламывал сегмент, ничего
  не получал и оставлял неремонтируемую дыру. Проброс реализован (и для игрока, и для взрыва);
  поиск колонны намеренно останавливается на первом не-филлере, чтобы не утащить второй реактор,
  стоящий ниже.
- **`BlastFurnaceExtensionBlock`** — `onPlace` при `!canSurvive` ломал сам себя; Create ставит
  половинки по одной, и первая уничтожала себя до появления второй. Внутри окна проверка
  пропускается (Sable сюда не попадал: он ставит блоки с `captureBlockSnapshots`).
- **Лог-спам** — `LevelChunkSilentRemovalMixin` и `UniversalMachinePartBlock` писали INFO на каждый
  подавленный блок: сборка корабля давала сотни строк в `latest.log`. Переведено на `debug`, как
  и остальные точки гварда.

**Проверено, вопросов нет:** 68 из 80 контроллеров сносят структуру корректно (`facing` везде из
старого `state`, защита `!state.is(newState.getBlock())` есть у всех); ещё 9 — заглушки 1×1×1
(`MultiblockStructureStubs.singleController()`), сносить нечего. Двери/шлюзы построены на
`UniversalMachinePart` и покрыты общим механизмом. PWR-части (`PWRPartBlock`) при сломе только
сбрасывают `assembled` — ни фантомов, ни дропов. Мельтдаун ZIRNOX не дюпает: инвентарь чистится
до свапа блока. Прямые `drops()` в обход guard-aware хелпера (14 машин) дюп не дают — внутри окна
`LevelChunkSilentRemovalMixin` глушит их `onRemove` целиком, и BE к тому моменту уже снят движком.

**Осталось решить владельцу (не трогал):**

1. **`CargoElevatorBlock`** — шахта 3×3 (`tryAddFloor` штампует слой и проставляет `corePos`), но
   ни `onRemove`, ни `playerWillDestroy`, ни релинка после переноса нет вообще: сломанная ячейка
   оставляет остальные 8, счётчик этажей у ядра врёт. Лифт и PWR используют свой интерфейс
   `IDummyCorePart`, которого не знает `SubLevelRelinkMixin`. Нужен либо перевод на общий механизм,
   либо своя логика — это уже дизайнерское решение.
2. **Взрыв по обшивке дропает станок мимо `dropFromExplosion`** — `UniversalMachinePartBlock`
   каскадит `destroyBlock(controllerPos, true)` с жёстким `true`. `MachineRefineryBlock` и
...
## BO. Вёрстка GUI машин: смещённые слоты и обрезанные окна

Репорт из игры: «в некоторых машинах криво выглядит инвентарь». Проверка сделана объективно,
без глазомера: написан декодер PNG на perl (`/tmp/PNGRead.pm`), который для каждой из 148 GUI-текстур
находит **нарисованную** сетку слотов — горизонтальные тёмные границы ячеек, идущие через всю
инвентарную полосу `x=7..168`. Строка сетки с координатой Y означает, что верхний левый угол слота
равен Y+1. Затем это сверено с координатами слотов игрока во всех 139 меню и с `imageWidth`/
`imageHeight` экранов. Эталон дополнительно подтверждён по оригиналу 1.7.10: `xSize`/`ySize` из
`com/hbm/inventory/gui/GUI*.java` и координаты слотов из `com/hbm/inventory/container/Container*.java`.
Там, где оригинал есть, оба метода дали одно и то же.

Полезные соотношения: `imageHeight = hot + 24`, `main = hot - 58`, `inventoryLabelY = imageHeight - 94`.

Итог: из 110 меню с инвентарём **77 были в порядке**, 15 имели смещённые слоты, 7 — обрезанное окно.

### BO1. Смещённый инвентарь (исправлено)

| машина | было | стало | источник эталона |
|---|---|---|---|
| SILEX | 176×204, 122/180 | 176×**222**, **140/198** | `GUISILEX` 176×222, `ContainerSILEX` |
| Турель | 122/180 | **140/198** | `GUITurretBase`, `ContainerTurretBase` |
| РБМК: стержень управления и авто-стержень | 84/142 | **104/162** | `ContainerRBMKControl(+Auto)`, смещение +20 |
| Разливочная машина (strand caster) | 176×186, 104/162 | 176×**214**, **132/190** | `ContainerMachineStrandCaster` |
| FSTBMB | 176×166, 84/142 | 176×**222**, **140/198** | `ContainerNukeFstbmb`; слоты бомбы (62,36)/(98,36) → **(17,36)/(53,36)** |
| Кастомная бомба | 176×166, 86/144 | 176×**222**, **140/198** | `ContainerNukeCustom` |
| Печь-нагреватель | 176×166, 84/142 | 176×**168**, **86/144** | своя текстура (в оригинале GUI нет) |
| РТГ-дифурнейс | 176×184, 103/161 | 176×**166**, **84/142** | `GUIMachineDiFurnaceRTG` |
| Газовый факел | 176×204, 122/180 | 176×**203**, **121/179** | `ContainerMachineGasFlare`, offset 37 |
| FEL | 176×204, 122/180 | **203×169**, **83/141** | `GUIFEL`, `ContainerFEL` |
| Электролизёр | 176×176, 104/162 | **210×247**, **165/223** | `GUIElectrolyser`, `ContainerElectrolyser` |
| PWR-контроллер | 226×230, 147/205 | **176×188**, **106/164** | `GUIPWR`, `ContainerPWR` |
| Подстанция | 84/142 на текстуре газовой турбины | текстура заменена на `gui_linker.png` (панель ровно 176×166) | — |

Отдельно: у РТГ-дифурнейса слоты машины тоже стояли мимо нарисованных рамок — входы перенесены
на (80,18)/(80,54), выход на (134,36), а шесть RTG-пеллет из ряда по `y=71` (он налезал на инвентарь
игрока) — в сетку 2×3 на (22..40, 18..54), как в оригинале.

У PWR-контроллера порт был свёрстан под окно 226×230, которого не существует: текстура — апстримная
панель 176×188. Слоты топлива переставлены на (53,5) и (89,32), баки — на (8,5)/(26,5) 16×52,
полоса нагрева — на (54,33) 34×14, кнопки регулировки стержней перенесены в свободную зону (116/140, 64).

### BO2. Обрезанное окно (исправлено)

Объявленная высота была меньше нарисованной панели, из-за чего нижний ряд слотов оказывался
за пределами фона:

- компрессор: 176 → **204** (хотбар вываливался за фон на 28 px);
- рафинировочная: 231 → **232**; радар (слоты) и бочка отходов: 184 → **185** (срезалась нижняя
  граница рамки).

### BO3. Текстуры

`gui_keyforge.png` был единственным GUI-файлом, отличавшимся от оригинала: самодельная перерисовка
на 176×166 с сеткой 84/142, тогда как код (правильно) следовал оригинальным 176×186 и 104/162 —
отсюда сдвиг на 20 px. Возвращена апстримная текстура, код не тронут.

### BO4. Осталось владельцу

1. **Электролизёр**: окно и инвентарь выправлены, но 10 машинных слотов порта (батарея, ID жидкости,
   побочные продукты, кристалл, выходы металла) стоят по собственным координатам и не попадают на
   нарисованные рамки. В оригинале у этой машины ~20 слотов с другой семантикой
   (`ContainerElectrolyser`: жидкостная секция (6/24/78/134 × 18/54), металлическая (10,90)(37,122)
   (60,112)(98,112), выходы 136/154 × 86/104/122, правая колонка (186,109)(186,140)(186,158)).
   Раскладка требует решения по семантике слотов нашего порта.
2. **FEL, газовый факел, SILEX** — три копии одного шаблона: у всех трёх меню **нет ни одного
   машинного слота**, хотя в оригинале они есть (FEL — 2, факел — 6, SILEX — 11). Это пропуск порта,
   а не вёрстки.
3. **Кирпичная печь**: в оригинале есть четвёртый слот (`SlotCraftingOutput` на (35,53)), у нас его нет.
4. **Компрессор**: в оригинале два слота апгрейдов на (52,72) и (70,72), у нас их нет.
5. Дубликаты текстур: `gui/gui_heating_oven.png` = `gui/machine/gui_heating_oven.png`,
   `gui/gui_silex.png` = `gui/processing/gui_silex.png` — код грузит вторые, первые мертвы.

## BP. Ревью сессии: деградации и упрощение

Двухчастное ревью изменений `a12a5c58c..HEAD` (мультиблоки, ресурсы, вёрстка GUI).

### BP1. Деградации — исправлено

Из десяти находок шесть — регрессии, внесённые правками этой же сессии.

- **`UniversalMachinePartBlock.onRemove`**: перерезолв контроллера, добавленный ради мультиблоков
  в саблевеле, срабатывал и внутри каскада `destroyStructure`. Контроллер там уже стёрт, поэтому
  каждая часть уходила в радиусный поиск по кубу 49³ — для Watz (74 части) это ~8.7 млн обращений
  к миру на один слом. Перерезолв закрыт флагами каскада, а на пути снятия блока радиусный откат
  отключён совсем (`relinkOrphanedPartDeterministic(..., allowRadiusFallback = false)`): повторять
  попытку всё равно нечем, блок уже уходит.
- **`RedPylonCoreBlock.onRemove`**: под гард окна переноса ушёл и `disconnectAll()`, а это
  единственное место, которое вычищает позицию пилона из списков соединений соседей — у них
  оставался провод в пустоту без шансов на самоисцеление. Под гардом остался только `clearDummies`.
- **`BlastFurnaceExtensionBlock`**: гард стоял в `neighborChanged`, а обратная установка
  контрапшена ходит через `updateShape`.
- **`RBMKColumnFillerBlock`**: искал колонну по дефолтной высоте вместо геймрула, и его новый
  `onRemove` сносил колонну прямо посреди `standardMelt` (там филлеры заменяются на обломки,
  а не на воздух). Проброс слома теперь только при замене на воздух.
- **`RBMKColumnBlock`**: перенос колонны с крышкой выдавал бесплатную крышку — `popResource`
  не проверял окно переноса.
- **Пилон и колонна РБМК, приехавшие движком сборки**, оставались без даммиков/филлеров навсегда:
  подавление было, восстановления не было. Оба до-заполняют пропуски отложенным тиком после
  закрытия окна.

По GUI три правки самой сессии оказались неверными: у FEL окно нельзя расширять до апстримных 203
(этот экран блитит свои полоски из правой колонки панели, она вылезла бы фоном); у РТГ-дифурнейса
строка прогресса после уменьшения окна легла на первый ряд инвентаря; у бочки отходов не хватило
одного пикселя (панель непрозрачна до 188). Кнопки стержней PWR переставлены на ячейки, которые
панель реально рисует.

Отклонены после самостоятельной проверки: ширина 210 у электролизёра верна (правая колонка — часть
панели, там нарисованы слоты, как в `ContainerElectrolyser`), а закрытие окна переноса по концу тика
безопасно — и Sable, и Create двигают блоки синхронно внутри тика, `SERVER_POST` приходит после
всего `tickChildren`.

### BP2. Упрощение — сделано

- Три новых дропа инвентаря переведены на общий `MachineDrops.dropInventory` (его зовут ещё
  31 машина); заодно клиентская проверка убрана в сам `BaseMachineBlockEntity.dropInventoryContents`,
  чтобы правило было одно на весь мод.
- `destroyStructure`: две волны (по позициям структуры и по габаритам) схлопнуты в один обход
  габаритов с объединённым условием. Вторая волна была надмножеством первой и повторяла её хвост.
- `ContraptionAssemblyGuard`: 30-секундный страховочный таймаут стал недостижим после `endServerTick`
  и удалён вместе со вторым ThreadLocal — остался один счётчик глубины.
- `RBMKColumnBlock`: три копии цикла заполнения филлеров сведены в `fillFillers`, отложенный тик
  планируется только внутри окна переноса (раньше — на любую установку, дублируя `setPlacedBy`).
- `RBMKColumnFillerBlock`: два скана вниз по колонне сведены в один `findBase(..., contiguousOnly)`.
- `UniversalMachinePartBlock.playerWillDestroy`: тело вынесено в `destroyControllerOnHarvest`,
  обе платформенные ветки стали однострочными (раньше правку приходилось вносить дважды).
- Мелочи: дублирующийся комментарий у пилона, неиспользуемый импорт в миксине, два экрана
  с литералом `"hbm_m"` вместо `RefStrings.MODID`.

### BP3. Упрощение — предложено, но не сделано (решение владельца)

1. **`@WrapMethod` вместо пары HEAD/RETURN** в `SubLevelMoveWindowMixin` и `ContraptionWindowMixin`.
   Это корневая форма: `try { original.call(...) } finally { pop(); }` переживает и отмену метода
   чужим миксином, и исключение внутри него. Тогда `endServerTick` останется страховкой, а не
   основным механизмом. MixinExtras на NeoForge 1.21.1 доступен из коробки, объём ~30 строк.
2. **Единое событие «перенос закончен»**. Сейчас восстановлением после переноса занимаются три
   разных механизма с разными константами: ретрай части мультиблока (60 попыток × 20 тиков),
   отложенный тик пилона и отложенный тик колонны РБМК. Если гвард будет копить затронутые позиции
   (их и так видит `LevelChunkSilentRemovalMixin`) и при закрытии окна звать
   `onContraptionMoveFinished`, все три схлопнутся в один прицельный проход. ~150 строк.
3. **Радиусный поиск контроллера** (`relinkOrphanedPart`, 49³ = 117 649 лукапов) стоит заменить
   обходом BlockEntity загруженных чанков: все 80 контроллеров — блоки с BE, кандидат обязан быть
   в `LevelChunk.getBlockEntities()`. Это сотни объектов вместо сотни тысяч лукапов и снимает
   нужду в проверке `hasChunk`.
4. **Вёрстка GUI**: правило `imageHeight = hot + 24`, `main = hot - 58`,
   `inventoryLabelY = imageHeight - 94` соблюдается во всех парах, но живёт в двух файлах и нигде
   не выражено — отсюда и разъезды. Лечится `setWindow(w, h)` в `GuiInfoScreen` плюс
   `MenuLayouts.addPlayerInventory(this, inv, height)`; цикл 3×9 + хотбар сейчас скопирован
   в ~134 меню, так что массовую миграцию делать не стоит — только по мере касания файлов.
5. **Проверка полноты ресурсов в dev-режиме**: один проход по реестрам блоков и предметов с
   фильтром по namespace и сравнением с `getMissingModel()` ловил бы разом все шесть категорий
   пропусков, которые в этой сессии искались вручную. ~15 строк в `ClientSetup`.
6. **`GUIMachineFel` / `GUIMachineFlareStack` / `GUIMachineSilex`** — три посимвольные копии одного
   шаблона (и меню тоже). Координаты оверлеев у всех трёх одинаковые, хотя высоты окон теперь
   разные (165 / 203 / 222), то есть совпасть может максимум одна. Правильное лечение — общий
   базовый экран и порт машинных слотов, которых у всех трёх нет (см. BO4).
7. **Цикл регистрации моделей ракет** в `ClientSetup`, возможно, не нужен целиком:
   `MissileRenderHelper.resolveBakedModel` читает штатную инвентарную модель, а не тот ключ,
   который цикл регистрирует. Стоит либо убрать цикл, либо перевести чтение на
   `PlatformHooks.getItemModel`, чтобы ключ считался в одном месте.

### BP4. Ревью на деградации после упрощения

Проверка рефакторинга из `ceb469974` — поведение задумывалось неизменным.

**Схлопывание двух проходов `destroyStructure` в один — эквивалентно.** Перебраны все комбинации
«позиция в наборе структуры или вне его × блок наш или чужой × BE нет / не часть / часть ×
владелец `null` / наш / чужой живой / чужой мёртвый»: результат по сносу и по отписке от энергосети
совпадает во всех случаях. Множество опрашиваемых позиций не выросло (старая вторая волна и так
ходила по тем же габаритам), обращений к миру стало меньше, `finally` со сбросом `IS_DESTROYING`
отрабатывает и на раннем `return` при пустой схеме, поиск `MutableBlockPos` в `HashSet<BlockPos>`
корректен (`equals`/`hashCode` объявлены в `Vec3i` и не переопределены).

**Четыре расхождения, внесённых упрощением, исправлены:**

1. `RBMKColumnBlock.setPlacedBy` стал пропускать занятые позиции — раньше филлеры ставились
   безусловно. Проверки свободного места при установке колонны нет нигде, так что колонну можно
   поставить под потолком или поверх обломков после мельтдауна и получить дыру в колонне:
   пропадает коллизия сегмента, ломается проброс слома и выбор верхнего филлера для крышки.
   Пропуск занятого оставлен только в восстановительном тике, где он и нужен.
2. Отложенный тик планировался только внутри окна переноса, из-за чего колонна, поставленная
   `/setblock`, структурой, генератором мира или другим модом, оставалась без филлеров вовсе.
   Планируется снова на любую установку.
3. `relinkOrphanedPartDeterministic(..., false)` в `onRemove` части гасил не только радиусный
   поиск, но и саму починку, ради которой вызов добавлялся. Причина глубже: `onRemove` вызывается
   уже после перезаписи секции чанка, поэтому `getBlockState` возвращает воздух, `FACING`
   молча уходит в `NORTH`, и детерминированный путь верен ровно для одной ориентации из четырёх.
   Теперь состояние части передаётся в хелпер параметром (в `onRemove` оно на руках), так что
   детерминированный путь работает при любом повороте, а радиусный откат остаётся разрешён —
   он отключается только на время самоуборки фантома (`MultiblockStructureHelper.isOrphanCleanup()`),
   где поиск только что провалился шестьдесят раз подряд.
4. `findBase` у филлера РБМК стал сканировать на блок глубже и для путей взаимодействия
   (`useWithoutItem`, `getCollisionShape`), где осиротевший филлер мог привязаться к колонне ниже.
   Дополнительная глубина оставлена только режиму проброса слома.

**Проверено и эквивалентно:** переход трёх машин на `MachineDrops.dropInventory` (тот же набор
предметов, порядок относительно сноса структуры сохранён; у Watz дроп перестал выполняться ещё и
на клиенте — это исправление), клиентская проверка внутри `dropInventoryContents` (все 55
вызывающих — серверные пути), вынос тела `playerWillDestroy` в общий метод (построчно совпадает,
обе платформенные ветки компилируются), удаление мёртвого импорта и замена литерала `"hbm_m"`
на `RefStrings.MODID`.

Осталось известным риском: удаление 30-секундного таймаута окна переноса убрало страховку для
не-серверного потока. Оба миксина, открывающие окно, серверные, а все потребители сами отсекают
клиент, так что риск теоретический — но если `push()` когда-нибудь случится на клиентском потоке,
самозакрытия там больше нет.

## BQ. Закрытие пункта 1: порт подхода 1.7.10 (лифт, взрывы, слоты SILEX)

Сверка с оригиналом: `com/hbm/blocks/BlockDummyable.java`, `com/hbm/blocks/machine/BlockCargoElevator.java`,
`com/hbm/inventory/container/ContainerSILEX.java`, `com/hbm/blocks/machine/MachineRefinery.java`.

### BQ1. Грузовой лифт

В оригинале лифт — обычный `BlockDummyable`: ядро плюс восемь даммиков того же блока, у каждого
в мете направление к ядру. Отсюда всё поведение:

- **Установка** ставит сразу слой 3×3 (`getDimensions {0,0,1,1,1,1}`), а если кольцо не влезает —
  установка отменяется и предмет возвращается игроку (`BlockDummyable.onBlockPlacedBy`).
- **Слом любой ячейки** через мету уводит цепочку к ядру, а `destroyIfOrphan` в
  `onNeighborBlockChange` доламывает остальные: даммик, потерявший соседа в сторону ядра, удаляет
  себя сам. Причём удаление идёт `setBlockToAir`, то есть без дропа.
- **Дроп** считает `BlockCargoElevator.getDrops`: `height + 1` предметов, разложенных по стопкам, —
  ровно столько, сколько стоила постройка (по одному за этаж).

У нас было: одиночный блок при установке (без нижнего кольца), ни каскада сноса, ни уборки
осиротевших ячеек, ни правильного дропа — сломал ячейку, остальные восемь висят, счётчик этажей
у ядра врёт.

Портировано в том же виде, с поправками на 1.21.1:

- `setPlacedBy` ставит нижнее кольцо 3×3 и отменяет установку с возвратом предмета, если оно
  не влезает;
- `playerWillDestroy` и `onRemove` сносят всю шахту (`destroyShaft`), ячейки удаляются без дропа,
  а недостающие `height` предметов выбрасываются стопками по 64 — итог совпадает с апстримным
  `height + 1`, потому что сама сломанная ячейка дропается лут-таблицей;
- **поправка на Sable/Create**: движки сборки переносят ячейки, не трогая сохранённый указатель на
  ядро, поэтому после переезда все указатели врут. Вместо меты направления (её в 1.21 нет) ядро
  перевыводится из геометрии самой шахты — спуск по непрерывной колонне вниз и поиск ядра в 3×3
  на нижнем уровне (`findCoreByGeometry`). Проверка идёт раз в 40 тиков на ячейку;
- ячейка без ядра удаляет себя (аналог `destroyIfOrphan`), но только когда чанк названной позиции
  загружен — ровно та же оговорка, что и в комментарии оригинала про границы чанков;
- весь каскад и уборка молчат внутри окна переноса контрапшена.

### BQ2. Взрыв по обшивке мультиблока

В оригинале машина — один блок, поэтому взрыв по любой её клетке вызывает её собственный
`onBlockExploded`: рафинировочная загорается и не дропается (`dropFromExplosion → false`),
второй взрыв добивает руины. У нас обшивка — отдельный блок `UniversalMachinePartBlock`, и
взрыв, попавший в обшивку, шёл мимо этой логики: каскад сноса дропал станок обычным предметом.

Теперь `UniversalMachinePartBlock.onBlockExploded` передаёт взрыв блоку-контроллеру и выходит —
дальше контроллер решает сам (руины у рафинировочной и танка, обычное разрушение у остальных),
а снос остальной структуры идёт его штатным каскадом. Повторные попадания того же взрыва в
соседние обшивки безвредны: контроллера там уже нет, и они удаляются обычным путём.

### BQ3. Слоты SILEX

У `MachineSilexBlockEntity` три слота (вход, выход, батарея), а меню не добавляло **ни одного** —
машину нельзя было загрузить руками, только автоматикой. Координаты взяты по оригинальному
`ContainerSILEX` и сверены с рамками, нарисованными на текстуре: вход (80,12), выход (116,90),
батарея (8,24). Заодно реализован `quickMoveStack`, который раньше возвращал пустой стек: shift-клик
кладёт в батарею и вход, но никогда в выход.

Проверено и правки не потребовалось: у кирпичной печи и компрессора все слоты их BE в меню есть.

### BQ4. Осталось

`MachineFelBlockEntity` и `MachineFlareStackBlockEntity` заведены с инвентарём на **ноль** слотов,
тогда как в оригинале у FEL их 2, у газового факела 6. Это не вёрстка и не пропуск в меню —
слоты там нечем наполнить, нужен порт самой механики машин. Вынесено отдельным пунктом.

## BR. Недоделанные системы: второй заход

Продолжение инвентаризации «что в моде ещё не готово». Каждый пункт сверен с оригиналом 1.7.10,
собран на обеих платформах и прогнан через 287 геймтестов.

### BR1. Опасность отработанных пластин

`ModItems.WASTE_PLATE_*` были зарегистрированы **без единой записи в `HazardSystem`** — отработанное
топливо в руках не облучало вообще. Добавлены записи по формуле оригинала (`base = 15 * mult`,
у нулевой меты 7.5 %): U233 13, U235 10, MOX 16, PU239 13.5, SA326 10, плюс отдельные
`RA226BE` (75·3·0.1·3) и `PU238BE` (10·3·0.1).

### BR2. Побочные продукты рафинировочной

Три из четырёх выходов возвращали `ItemStack.EMPTY` с пометкой «ещё не портировано», хотя
`OIL_TAR_CRACK` и `OIL_TAR_PARAFFIN` в моде есть. Выходы подключены, комментарии сняты; заодно
добавлены пять апстримных рецептов кристаллизатора на смолу (крудойл+хлор→воск, крэк+хлор→воск,
парафин+хлор→воск, воск+ионгель→заряженная гранула, парафин+эстрадиол→красная пилюля).

### BR3. Топливо стартовой площадки

Проверка `hasFuel` и слив топлива в `finalizeLaunch()` были закомментированы — ракеты стартовали
с пустых баков. Восстановлено по `TileEntityLaunchPadBase`: оба бака должны держать `fuelCap`
ракеты, у твердотопливных `fuelCap = 0`, поэтому им жидкость по-прежнему не нужна.

### BR4. Машины без подключения к жидкостной сети

Пять машин имели баки, но не входили в сеть: паяльная станция (10 из 17 рецептов требуют жидкость,
включая продвинутую микросхему), дейтериевая башня, турбовентилятор, излучатель и приёмник ядра.
Все переведены на `IFluidStandardReceiverMK2` с `trySubscribe` по шести сторонам раз в 20 тиков;
у четырёх последних заодно убран forge-only `getCapability`, который на 1.21.1 просто не вызывался.

### BR5. Насосная качалка

`GUIMachinePumpjack` брал текстуру `processing/gui_derrick.png`, которая побайтово совпадает с
`gui_mixer.png` и `gui_cracking_tower.png` — то есть это чужая панель-заглушка, и рисовалась она
со сдвигом (панель занимает x=14..161, а окно объявлено 176 широким). Меню при этом не добавляло
**ни одного** машинного слота из восьми, что были у BE.

В оригинале деррик и качалка делят один экран (`GUIMachineOilWell` + `ContainerMachineOilWell`),
поэтому экран и меню качалки сведены к копии дерриковых: текстура `machine/gui_well.png`, окно
176×166, слоты батареи (8,53), канистр (80,17)/(80,53), баллонов (125,17)/(125,53) и трёх
апгрейдов (152,17/35/53), плюс реализован `quickMoveStack`, который раньше возвращал пустой стек.

### BR6. Афтербёрнер фрекинговой вышки

`applyAfterburnerEffect()` был пустой заглушкой с TODO, а звался только в момент добычи. В
оригинале афтербёрнер живёт в тике базового класса бура и жжёт попутный газ **каждый тик**:
`min(газ, level*10)` мБ, `+5` энергии за мБ. Портировано как `burnAfterburnerGas()` в тике.

Заодно разделены уровни апгрейдов: `afterburnerUpgradeLevel` складывался с овердрайвом, из-за чего
овердрайв не влиял ни на что. Теперь он считается отдельно и работает как в оригинале —
делит задержку и умножает потребление на `overLevel + 1`.

### BR7. Зольник

У `MachineAshpitBlockEntity.addAsh()` не было **ни одного** места вызова: пять предметов золы
получить было нельзя. В оригинале зольник наполняет топка, стоящая прямо над ним. Добавлены
`feedFromBurner` и `ashFromFuel` (порт `TileEntityFireboxBase.getAshFromFuel`, где ore-dict имена
заменены на теги и id предметов), вызовы — из топки и нагревательной печи.

### BR8. Зола кирпичной печи

Комментарий в `MachineFurnaceBrickBlockEntity` объяснял, что четвёртый слот оригинала не портирован,
потому что в моде нет `powder_ash` и `EnumAshType`. Они появились вместе с зольником, так что
пометка устарела: добавлены слот `SLOT_ASH` (35,53, только на выемку — координата сверена с рамкой
на текстуре), счётчики золы по сортам и выдача пудры по порогу 2000, как в `TileEntityFurnaceBrick`.

### BR9. Апгрейды компрессора

Компрессор был заведён без слотов апгрейдов «для простоты». В оригинале их два, (52,72) и (70,72) —
обе рамки на нашей текстуре нарисованы. Добавлены слоты и расчёт из
`TileEntityMachineCompressorBase`: без рецепта скорость даёт 10/20/60 тика, с рецептом —
`duration / (speed+1)`; потребление `base / (power+1) * (over*2+1)`, время делится на `over+1`.
Заодно машина наконец учитывает `duration` рецепта, а полоса прогресса делится на фактическое
время цикла, а не на константу.

### BR10. `ExplosionNukeSmall`

Класса не было, его логика лежала приватной копией внутри ядерного крипера, поэтому НЛО при смерти
делал только ванильный взрыв (в оригинале следом идёт `ExplosionNukeSmall.PARAMS_MEDIUM`). Логика
вынесена в `com.hbm_m.explosion.ExplosionNukeSmall` с параметрами `PARAMS_SAFE/LOW/MEDIUM`; крипер
и НЛО переведены на неё. `PARAMS_HIGH` не переносился — это ветка «Толстяка», она и так идёт через
`NuclearExplosionAPI`; `PARAMS_TOTS` тоже, его частица `tinytot` не портирована.

### BR11. Осталось

- Дубликаты GUI-текстур: `processing/gui_derrick.png` теперь никем не используется, а
  `processing/gui_well.png` дублирует `machine/gui_well.png`. Наши `gui_mixer.png` и
  `gui_cracking_tower.png` — та же самая заглушка, и от апстримного `gui_mixer.png` она отличается
  на 13 389 пикселей. Экраны миксера и крекинг-башни стоит сверить отдельно.
- 15 типов рецептов без категории JEI (mold_casting 252, pyro_oven 69, solidification 36,
  coker 33, liquefactor 25, fraction_tower 20, breeder 11, catalytic_reformer 10, hydrotreater 7,
  molten_alloy 6, mixer 4, silex 4, vacuum_distill 3, combination_oven 1); у `rad_gen` рецептов нет вовсе.
- `BeamPronter` (лучи RAD-Beast и НЛО, ~180 строк) и ядро ДФК со стабилизаторами (~658 строк,
  оживило бы три уже портированные машины ДФК).

...
- **`EntityProcessorStandard` — потерянные `sqrt` и спад урона.** Оригинал:
  `double distanceScaled = entity.getDistance(x, y, z) / size;` (линейное расстояние) и
  `double knockback = (1.0D - distanceScaled) * density;`. Порт считал `distanceToSqr(...) / size`,
  то есть эффективный радиус схлопывался до `sqrt(size)` (у золотого крипёра 7 → 2.65 блока), и
  брал `knockback = density` без множителя `(1 - distanceScaled)`, из-за чего урон вообще не спадал
  с расстоянием. Соседний `EntityProcessorCross:129` всё это время считал правильно — то есть это
  описка в одном из двух процессоров, а не решение. Дампенер зачарований накладывается сверху, как
  в ванили.
...
- **Кассетные ракеты били не в ту сторону.** `ExplosionChaos.cluster` — дословная копия конструктора
  `EntityBulletBaseMK4`, включая `yaw * 180F / π` и отрицание pitch, поэтому контракт метода —
  **радианы**. А `MissileTier1/2/3` передавали `getYRot()`/`getXRot()`, то есть градусы: две
  конверсии взаимно уничтожались и `sin` получал градусы как радианы. Разброс
  `(float) Math.PI * 0.25F` (45° в радианах) при этом был указан правильно. Исправлены вызовы,
  математика метода не тронута.
...
Варианты для тестового сервера: починить исходящую сеть контейнера (корень); либо заставить запрос
падать мгновенно, направив `adastra.terrarium.earth` в `127.0.0.1` через `--add-host` — Ad Astra
поймает отказ и поедет дальше без списка станций; либо убрать Ad Astra.

Для контраста: наш сетевой запрос живёт на демон-потоке и (после P1) с таймаутами, то есть
заблокировать старт не может в принципе.

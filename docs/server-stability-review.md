...
- `WasteBlastGenerator` — `calculateSurvivalChance` получает манхэттенское расстояние вместо
  `maxRadius`; сортировка ломает модель затенения (T3).

### Приоритет 4 — гигиена и мелочи

- `EntityWormBase` не сериализует `headID` вовсе: после перезагрузки сегменты теряют голову (R).
- `SynchedEntityData` пишется на клиенте: `SpearEntity:97,101`, `VortexEntity:34` (N2).
- `StructureFoundationProcessor` инертен (перегрузка вместо переопределения) и ни в одном
  `processor_list` не упомянут (T3).

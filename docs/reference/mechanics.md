...
| жидкостей | 164 |
| типов рецептов | 38, суммарно 1383 рецепта |
...
- **Рецепты** — data-driven, 38 типов (`recipe/`, регистрация в `recipe/ModRecipes`), 1383 записи.
  ...
  (там почти весь `arc_furnace` — 36 из 37 — и `combination_oven` — 30 из 31). При подсчётах надо
  смотреть оба каталога. Кросс-версионный доступ — `platform/recipe/RecipeHooks`.

...
public final class SableClientCompat {
    ...
    /**
     * Position of {@code cameraPos} expressed in the plot-grid coordinates of the ship holding
     * {@code shipPos}; null when that block is not on a ship. Sable's outline mixin pushes the ship
     * render pose onto the highlight PoseStack before firing {@code RenderHighlightEvent}, so overlays
     * drawn from that event must be given in these plot-grid, camera-relative coordinates - applying
     * {@link #localToView} there would double-transform them (rotation applied twice).
     */
    @Nullable
    public static Vec3 plotCamera(BlockPos shipPos, Vec3 cameraPos) {
        if (!SableCompat.isLoaded()) return null;
        return Impl.plotCamera(shipPos, cameraPos);
    }
    ...
    private static final class Impl {
        ...
        @Nullable
        static Vec3 plotCamera(BlockPos pos, Vec3 cameraPos) {
            //? if >= 1.21.1 {
            /*try {
                var sub = dev.ryanhcode.sable.companion.SableCompanion.INSTANCE.getContainingClient(pos);
                if (sub == null) return null;
                return sub.renderPose().transformPositionInverse(cameraPos);
            } catch (Throwable t) {
                return null;
            }
            *///?} else {

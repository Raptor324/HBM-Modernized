...
/**
 * Разрушенный ZIRNOX — порт 1.7.10 {@code ResourceManager.zirnox_destroyed} (часть Plane).
 * <p>
 * World render: пустой chunk mesh — вся геометрия в BER/VBO
 * ({@link com.hbm_m.client.render.implementations.MachineZirnoxDestroyedRenderer}).
 * Item render: часть Plane с display-трансформами из JSON модели.
 */
public class MachineZirnoxDestroyedBakedModel extends AbstractMultipartBakedModel implements AbstractMultipartBakedModel.PartNamesProvider {

    private static final String[] PRIORITY = { "Plane" };

    private final String[] cachedPartNames;

    public MachineZirnoxDestroyedBakedModel(Map<String, BakedModel> parts, ItemTransforms transforms) {

    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            if (isPointInRect(TOGGLE_X, 98, 8, 8, (int) mouseX, (int) mouseY)) {
                playClickSound();
                ModPacketHandler.INSTANCE.sendToServer(new UpdateRadarC2SPacket(radar.getBlockPos(), 0));
                return true;
            }
            if (isPointInRect(TOGGLE_X, 108, 8, 8, (int) mouseX, (int) mouseY)) {
                playClickSound();
                ModPacketHandler.INSTANCE.sendToServer(new UpdateRadarC2SPacket(radar.getBlockPos(), 1));
                return true;
            }
            if (isPointInRect(TOGGLE_X, 118, 8, 8, (int) mouseX, (int) mouseY)) {
                playClickSound();
                ModPacketHandler.INSTANCE.sendToServer(new UpdateRadarC2SPacket(radar.getBlockPos(), 2));
                return true;
            }
            if (isPointInRect(TOGGLE_X, 128, 8, 8, (int) mouseX, (int) mouseY)) {
                playClickSound();
                ModPacketHandler.INSTANCE.sendToServer(new UpdateRadarC2SPacket(radar.getBlockPos(), 3));
                return true;
            }
        }

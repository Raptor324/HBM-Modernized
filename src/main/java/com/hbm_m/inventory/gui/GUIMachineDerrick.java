...
public class GUIMachineDerrick extends GuiInfoScreen<MachineDerrickMenu> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        int power = (int) (derrick.getEnergyStored() * 52L / Math.max(derrick.getMaxEnergyStored(), 1L));
        if (power > 52) {
            power = 52;
        }
        if (power > 0) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawCustomInfoStat(guiGraphics, mouseX, mouseY,
            ...
            this.leftPos + 78, this.topPos + 67,
                ...
                Component.literal("   " + derrick.getProgress() + " / " + derrick.getMaxProgress()));

...
public class GUIMachineSolderingStation extends GuiInfoScreen<MachineSolderingStationMenu> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        int power = (int) (solderingStation.getEnergyStored() * 52L / Math.max(solderingStation.getMaxEnergyStored(), 1L));
        if (power > 52) {
            power = 52;
        }
        if (power > 0) {
        ...
        int progress = solderingStation.getProgressScaled(33);
        if (progress > 0) {
            guiGraphics.blit(TEXTURE, this.leftPos + 72, this.topPos + 37, 192, 0, progress, 14);
        }

        if (solderingStation.getEnergyStored() > 0) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawElectricityInfo(guiGraphics, mouseX, mouseY,
            152, 18, 16, 52,
            solderingStation.getEnergyStored(), solderingStation.getMaxEnergyStored());

        drawCustomInfoStat(guiGraphics, mouseX, mouseY,
            this.leftPos + 78, this.topPos + 67, 8, 8,
            this.leftPos + 78, this.topPos + 67,
                Component.literal("Progress:"),
                Component.literal("   " + solderingStation.getProgress() + " / " + solderingStation.getMaxProgress()));

...
public class GUIMachineRbmkConsole extends GuiInfoScreen<MachineRbmkConsoleMenu> {

    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(RefStrings.MODID, "textures/gui/reactors/gui_rbmk_console.png");

    private final MachineRbmkConsoleBlockEntity rbmkConsole;

    public GUIMachineRbmkConsole(MachineRbmkConsoleMenu menu, Inventory playerInventory, Component title) {
        super(menu, playerInventory, title);
        this.rbmkConsole = menu.getBlockEntity();
        this.imageWidth = 176;
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        int power = (int) (rbmkConsole.getEnergyStored() * 52L / Math.max(rbmkConsole.getMaxEnergyStored(), 1L));
        if (power > 52) {
            power = 52;
        }
        if (power > 0) {
        ...
        int progress = rbmkConsole.getProgressScaled(33);
        if (progress > 0) {
            guiGraphics.blit(TEXTURE, this.leftPos + 72, this.topPos + 37, 192, 0, progress, 14);
        }

        if (rbmkConsole.getEnergyStored() > 0) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawElectricityInfo(guiGraphics, mouseX, mouseY,
                152, 18, 16, 52,
                rbmkConsole.getEnergyStored(), rbmkConsole.getMaxEnergyStored());

        drawCustomInfoStat(guiGraphics, mouseX, mouseY,
                this.leftPos + 78, this.topPos + 67, 8, 8,
                this.leftPos + 78, this.topPos + 67,
                Component.literal("Progress:"),
                Component.literal("   " + rbmkConsole.getProgress() + " / " + rbmkConsole.getMaxProgress()));

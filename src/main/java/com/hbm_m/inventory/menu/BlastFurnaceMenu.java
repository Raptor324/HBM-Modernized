...
public class BlastFurnaceMenu extends AbstractContainerMenu {
    ...
    public BlastFurnaceMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        if (!(entity instanceof BlastFurnaceBlockEntity be)) {
        ...
        this.addSlot(new FuelSlot(container, BlastFurnaceBlockEntity.FUEL_SLOT, 80, 81)); // Топливо
        this.addSlot(new Slot(container, BlastFurnaceBlockEntity.INPUT_SLOT_FIRST, 80, 27)); // Первый вход
        this.addSlot(new Slot(container, BlastFurnaceBlockEntity.INPUT_SLOT_SECOND, 80, 45)); // Второй вход
        this.addSlot(new OutputSlot(container, BlastFurnaceBlockEntity.OUTPUT_SLOT_FIRST, 134, 72)); // Основной выход
        this.addSlot(new OutputSlot(container, BlastFurnaceBlockEntity.OUTPUT_SLOT_SECOND, 134, 90)); // Шлак
    ...
    public int getScaledFuelProgress(int pixelSize) {
        return (int) ((long) this.data.get(DATA_INDEX_FUEL) * pixelSize / BlastFurnaceBlockEntity.MAX_FUEL);
    }

    public int getScaledAirProgress(int pixelSize) {
        return (int) ((long) this.data.get(DATA_INDEX_AIR) * pixelSize / BlastFurnaceBlockEntity.AIR_CAPACITY_MB);
    }

    public int getScaledFlueProgress(int pixelSize) {
        return (int) ((long) this.data.get(DATA_INDEX_FLUE) * pixelSize / BlastFurnaceBlockEntity.FLUE_CAPACITY_MB);
    }
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (index < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            ...
            if (BlastFurnaceBlockEntity.isFuel(sourceStack)) {

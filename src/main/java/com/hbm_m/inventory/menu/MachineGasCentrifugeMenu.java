...
public class MachineGasCentrifugeMenu extends AbstractContainerMenu implements ILongEnergyMenu {
    ...
    public MachineGasCentrifugeMenu(int containerId, Inventory playerInventory, MachineGasCentrifugeBlockEntity blockEntity, ContainerData data) {
        ...
        var itemHandler = this.blockEntity.getInventory();

        // Battery slot (top-right area, near energy bar)
        this.addSlot(new SlotItemHandler(itemHandler, BATTERY_SLOT, 154, 4) {
            @Override
        ...
        this.addSlot(new SlotItemHandler(itemHandler, INPUT_SLOT_1, 45, 17));

        // Input slot 2 (right input area)
        this.addSlot(new SlotItemHandler(itemHandler, INPUT_SLOT_2, 69, 17));

        // Output slot 1
        this.addSlot(new SlotItemHandler(itemHandler, OUTPUT_SLOT_1, 45, 53) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
        ...
        // Output slot 2
        this.addSlot(new SlotItemHandler(itemHandler, OUTPUT_SLOT_2, 69, 53) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }

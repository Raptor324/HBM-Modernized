...
/**
 * Шрабидиевые слитковые блоки с {@code ExtDisplayEffect.SCHRAB} (GIT {@link com.hbm.blocks.generic.BlockHazard}).
 */
public class SchrabDisplayBlock extends Block {

    public SchrabDisplayBlock(Properties properties) {
        super(properties);
    }

    @Override
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource rand) {
        spawnSchrabFogParticles(level, pos, rand);
    }

    /** Порт {@link com.hbm.blocks.generic.BlockHazard#sPart} + {@code ExtDisplayEffect.SCHRAB}. */
    static void spawnSchrabFogParticles(Level level, BlockPos pos, RandomSource rand) {
        if (rand.nextInt(4) != 0) {
            return;
        }

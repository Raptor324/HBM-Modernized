...
/** Port of {@code com.hbm.blocks.network.BlockConveyorDouble} (1.7.10 Original) - two parallel lanes. */
public class ConveyorDoubleBlock extends ConveyorBendableBlock {

    public ConveyorDoubleBlock(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public Vec3 getClosestSnappingPosition(Level level, BlockPos pos, Vec3 itemPos) {
        ...
        if (dir.getStepX() != 0) {
            posX = clampedX;
            posZ += itemPos.z > posZ ? 0.25 : -0.25;
        }
        if (dir.getStepZ() != 0) {
            posZ = clampedZ;
            posX += itemPos.x > posX ? 0.25 : -0.25;
        }
